...
def compare_json_files(original_folder, upper_bound_folder, output_file):
    ...
    firstOriginalTallest = True
    firstOtherTallest = True
    for i in range(len(sorted_file_names)):
        if sorted_original_means[i] > sorted_upper_bound_means[i]:
            if firstOriginalTallest:
                plt.bar(i, sorted_original_means[i], color='grey', edgecolor='black', alpha=0.5, label='UpperBound015')
                plt.bar(i, sorted_upper_bound_means[i], color='black', edgecolor='black', alpha=0.5, hatch='//')
                firstOriginalTallest = False
            else:
                plt.bar(i, sorted_original_means[i], color='grey', edgecolor='black', alpha=0.5)
                plt.bar(i, sorted_upper_bound_means[i], color='black', edgecolor='black', alpha=0.5, hatch='//')
        else:
            if firstOtherTallest:
                plt.bar(i, sorted_upper_bound_means[i], color='white', edgecolor='black', alpha=0.5, hatch='//', label='G1BoundingBox02')
                plt.bar(i, sorted_original_means[i], color='black', edgecolor='black', alpha=0.5)
                firstOtherTallest = False
            else:
                plt.bar(i, sorted_upper_bound_means[i], color='white', edgecolor='black', alpha=0.5, hatch='//')
                plt.bar(i, sorted_original_means[i], color='black', edgecolor='black', alpha=0.5)
    # plt.bar(range(len(sorted_file_names)), sorted_original_means, color='grey', edgecolor='black', alpha=0.5, label='Original')
    # plt.bar(range(len(sorted_file_names)), sorted_upper_bound_means, color='white', edgecolor='black', alpha=0.5, label='Other', hatch='//')
    plt.xlabel('Query')
    plt.ylabel('Mean Value (s)')
    plt.title('Run Time Comparison without Histograms')
...
original_folder = "test-results/upper_bound_4_levels_incremental_0_15/indexes"
#original_folder = "test-results/upper_bound_fixed_0_15/histograms_and_indexes"
