...
#     # Plot each bar separately and adjust their heights based on the comparison of their mean values
#     legend_plotted = False  # Flag to track if legend has been plotted
#     for i in range(len(sorted_file_names)):
#         original_mean = sorted_original_means[i]
#         upper_bound_mean = sorted_upper_bound_means[i]

#         # Determine the height of each bar based on the minimum mean value
#         bar_height = min(original_mean, upper_bound_mean)

#         if (original_mean < upper_bound_mean):
#             # Plot the bars with appropriate hatch patterns
#             plt.bar(i, bar_height, color='grey', alpha=0.5, label='Original' if not legend_plotted else None, hatch='xx')
#             plt.bar(i, upper_bound_mean - bar_height, bottom=bar_height, color='lightgrey', alpha=0.5, label='Upper bound incr=0.15' if not legend_plotted else None, hatch='..')
#         else:
#             # Plot the bars with appropriate hatch patterns
#             plt.bar(i, bar_height, color='lightgrey', alpha=0.5, label='Upper bound incr=0.15' if not legend_plotted else None, hatch='..')
#             plt.bar(i, original_mean - bar_height, bottom=bar_height, color='grey', alpha=0.5, label='Original' if not legend_plotted else None, hatch='xx')
        
#         if not legend_plotted:
#             legend_plotted = True  # Update flag to indicate legend has been plotted once

#     plt.xlabel('Query')
#     plt.ylabel('Mean Value (s)')
#     plt.title('Run Time Comparison')
#     plt.xticks(range(len(sorted_file_names)), sorted_file_names, rotation=45, ha='right')
#     plt.legend()  # Plot the legend once outside the loop
#     plt.tight_layout()
#     plt.show()
